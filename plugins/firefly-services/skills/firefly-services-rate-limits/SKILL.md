...
- Wall-clock latency
- HTTP status
- `Retry-After` header on any `429` (the one rate-limit signal Adobe documents). If informational `X-RateLimit-*` headers happen to be present, log them too — but treat them as best-effort, not guaranteed
...
6. Observability captures the `Retry-After` header on every `429` (the documented signal); any `X-RateLimit-*` headers are logged opportunistically but not relied upon
