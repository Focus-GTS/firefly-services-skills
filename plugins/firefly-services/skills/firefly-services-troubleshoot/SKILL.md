...
| Error | Cause | Fix |
|---|---|---|
| `"message": "size width must be one of [..."]"` | Width/height not in the allowed list | Use one of the supported sizes (1024x1024, 1408x768, 768x1408, 2048x2048, etc. — see endpoint docs) |
...
| 8.4 | Pre-signed URLs expire (typically 1 hour). Generate just-in-time, not at job-creation time |
