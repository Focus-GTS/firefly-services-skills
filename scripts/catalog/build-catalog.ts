...
/**
 * Walk a directory recursively and return file paths (relative to root)
 * that match the given filename. We don't need full glob — every external
 * source we configure points at `SKILL.md` files specifically, so we just
 * look for that exact basename in any subdirectory the source allows.
 *
 * Node 20 doesn't ship a glob in node:fs/promises (Node 22+ does), so we
 * roll our own to keep CI compatibility.
 */
async function findSkillFiles(root: string, pattern: string): Promise<string[]> {
  // Pattern shape we support: `<dir>/**/skills/*/SKILL.md` or similar.
  // We simplify: find every SKILL.md under root, then filter against the
  // pattern's top-level prefix (the part before the first `*`).
  const stopBefore = pattern.indexOf("*");
  const requiredPrefix = stopBefore > 0 ? pattern.slice(0, stopBefore) : "";
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (!requiredPrefix || rel.startsWith(requiredPrefix)) {
          results.push(rel);
        }
      }
    }
...
async function scanExternalSource(source: ExternalSource): Promise<ExternalSkill[]> {
  ...
  for (const pattern of source.skill_globs) {
    const files = await findSkillFiles(root, pattern);
